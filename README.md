...
5. **RedClaw 自动化执行**：把单轮对话、技能调用、定时任务、长周期任务和后台 Runner 放在一个入口里持续运行。
6. **团队协作**：在“团队”里管理成员画像、成员知识、单成员对话和多人群聊协作。
7. **主体库**：统一沉淀人物、商品、场景等创作主体，后续在写稿、生图、封面时直接复用。
8. **媒体库**：统一管理 AI 生成图、导入图和计划图，支持按项目、稿件、来源快速过滤和绑定。
9. **封面工作台**：支持模板图 + 底图 + 标题组的封面生成，也支持让 AI 直接决定封面文案方向。
10. **后台持续运行**：支持定时、心跳、长周期等自动化模式，让选题、跟进和内容生产可以在后台延续。
