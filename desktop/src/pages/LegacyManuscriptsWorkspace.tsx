...
export function Manuscripts({ pendingFile, onFileConsumed, onNavigateToRedClaw, isActive = false }: ManuscriptsProps) {
    ...
    useEffect(() => {
        if (state.selectedFile) {
            ...
            const display = stripManuscriptExtension(name);
    ...
    const handleCreate = async (type: 'file' | 'folder') => {
        ...
        while (existingNames.has(type === 'file' ? ensureManuscriptFileName(name, MANUSCRIPT_MARKDOWN_EXTENSION) : name)) {
        ...
        try {
            if (type === 'folder') {
                ...
                const fileName = ensureManuscriptFileName(name, MANUSCRIPT_MARKDOWN_EXTENSION);
    ...
    const handleTitleRename = async () => {
        ...
        const currentStem = stripManuscriptExtension(currentName);

        if (titleValue.trim() === currentStem) return; // No change

        const newName = renameManuscriptKeepingExtension(currentName, titleValue.trim());
    ...
    const handleCopyWechatRichText = useCallback(async () => {
        ...
        if (copyResult?.success && state.selectedFile.endsWith(ARTICLE_DRAFT_EXTENSION)) {
            await window.ipcRenderer.invoke('manuscripts:save-formatted-html', {
                sourcePath: state.selectedFile,
                layoutHtml: formatResult.html,
                wechatHtml: formatResult.html,
            });
        }
    ...
    const handleRenameNode = async (oldPath: string, newName: string) => {
        ...
        const finalName = renameManuscriptKeepingExtension(currentName, newName);
    ...
    const renderNode = (node: FileNode, depth: number = 0) => {
        ...
        const displayName = stripManuscriptExtension(node.name);
