...
describe('new-track-zone-preview-store', () => {
  ...
  it('keeps state identity when setting the same previews again', () => {
    const ghostPreviews = [
      { left: 10, width: 50, label: 'clip-a', type: 'video' as const, targetZone: 'video' as const },
      { left: 10, width: 50, label: 'clip-a', type: 'audio' as const, targetZone: 'audio' as const },
    ];
  ...
  it('reuses unchanged per-zone preview arrays across updates', () => {
    ...
    expect(useNewTrackZonePreviewStore.getState().ghostPreviewsByZone.video).toBe(firstVideoPreviews);
  });
});
